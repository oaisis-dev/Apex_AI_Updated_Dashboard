...
export default function IntegrationsTab() {
  ...
  const handleConnect = (integrationId: string, category: string) => {
    setConnectedIntegrations(prev => {
      const newState = { ...prev };
      // Disconnect all integrations in the same category
      integrations
        .filter(int => int.category === category)
        .forEach(int => {
          newState[int.id] = false;
        });
      // Connect the clicked integration
      newState[integrationId] = true;
      return newState;
    });
  };

  const categoryIntegrations = getIntegrationsForCategory(selectedCategory);

  return (
    <div className="flex min-h-screen bg-white">
      
      {/* YouTube-style Left Sidebar */}
      <nav className="w-60 flex-shrink-0 h-screen sticky top-0 border-r border-slate-200 bg-white overflow-y-auto">
            ...
            {categoryIntegrations.map((integration) => (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-white border border-slate-200 rounded-lg p-1.5">
                    {integration.logoType === 'image' ? (
                      <img
                        src={integration.logo}
                        alt={integration.name}
                  ...
                  <span className="text-slate-900 text-sm font-medium">{integration.name}</span>
                </div>
                ...
                <div>
                  {connectedIntegrations[integration.id] ? (
                    ...
                    <button 
                      onClick={() => handleConnect(integration.id, integration.category)}
                      className="px-3 py-1.5 rounded-md border border-slate-300 text-slate-600 text-xs hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all duration-200 flex items-center justify-center gap-1.5 group"
                    >
